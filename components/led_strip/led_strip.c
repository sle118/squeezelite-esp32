...
static void led_strip_task(void* arg) {
    ...
    for(;;) {
        ...
        led_make_waveform(led_strip->led_strip_working, rmt_items, led_strip->led_strip_length);
        rmt_write_items(led_strip->rmt_channel, rmt_items, num_items_malloc, false);
    }

    if(rmt_items) { free(rmt_items); }
    vTaskDelete(NULL);
}
...
static bool led_strip_init_rmt(struct led_strip_t* led_strip) {
    ...
    if(cfg_ok != ESP_OK) { return false; }
    esp_err_t install_ok = rmt_driver_install(rmt_cfg.channel, 0, 0);
    if(install_ok != ESP_OK) { return false; }
...
bool led_strip_init(struct led_strip_t* led_strip) {
    ...
    if((led_strip == NULL) || (led_strip->rmt_channel >= RMT_CHANNEL_MAX) || (led_strip->gpio > GPIO_NUM_33) ||
        (led_strip->led_strip_working == NULL) || (led_strip->led_strip_showing == NULL) || (led_strip->led_strip_length == 0) ||
        (led_strip->access_semaphore == NULL)) {
        return false;
    }

    if(led_strip->led_strip_working == led_strip->led_strip_showing) { return false; }
    ...
    if(!init_rmt) { return false; }

    xSemaphoreGive(led_strip->access_semaphore);
    task_created = xTaskCreateStatic(led_strip_task, "led_strip_task", LED_STRIP_TASK_SIZE, led_strip, LED_STRIP_TASK_PRIORITY, xStack, xTaskBuffer);
...
bool led_strip_set_pixel_color(struct led_strip_t* led_strip, uint32_t pixel_num, struct led_color_t* color) {
    ...
    if((!led_strip) || (!color) || (pixel_num > led_strip->led_strip_length)) { return false; }
...
bool led_strip_set_pixel_rgb(struct led_strip_t* led_strip, uint32_t pixel_num, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    if((!led_strip) || (pixel_num > led_strip->led_strip_length)) { return false; }
