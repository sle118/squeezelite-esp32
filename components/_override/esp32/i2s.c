...
static float i2s_apll_get_fi2s(int bits_per_sample, int sdm0, int sdm1, int sdm2, int odir) {
    ...
    if(fout < SOC_I2S_APLL_MIN_FREQ || fout > SOC_I2S_APLL_MAX_FREQ) { return SOC_I2S_APLL_MAX_FREQ; }
...
static esp_err_t i2s_apll_calculate_fi2s(int rate, int bits_per_sample, int* sdm0, int* sdm1, int* sdm2, int* odir) {
    ...
    if(rate / bits_per_sample / 2 / 8 < APLL_I2S_MIN_RATE) { return ESP_ERR_INVALID_ARG; }
...
esp_err_t i2s_set_clk(i2s_port_t i2s_num, uint32_t rate, i2s_bits_per_sample_t bits, i2s_channel_t ch) {
     ...
     */
    if((p_i2s_obj[i2s_num]->mode & I2S_MODE_SLAVE) && !p_i2s_obj[i2s_num]->use_apll) { factor = 64 * bits; }
    ...
    if(!(p_i2s_obj[i2s_num]->mode & I2S_MODE_ADC_BUILT_IN)) { i2s_hal_set_rx_mode(&(p_i2s_obj[i2s_num]->hal), ch, bits); }
#else
    i2s_hal_set_rx_mode(&(p_i2s_obj[i2s_num]->hal), ch, bits);
#endif
    i2s_hal_set_tx_mode(&(p_i2s_obj[i2s_num]->hal), ch, bits);

    if(p_i2s_obj[i2s_num]->channel_num != (int)ch) { p_i2s_obj[i2s_num]->channel_num = (ch == 2) ? 2 : 1; }
    ...
    if((int)bits != p_i2s_obj[i2s_num]->bits_per_sample) {
        ...
        if(p_i2s_obj[i2s_num]->mode & I2S_MODE_TX) {
            ...
            if(save_tx) { i2s_destroy_dma_queue(i2s_num, save_tx); }
        }
        ...
        if(p_i2s_obj[i2s_num]->mode & I2S_MODE_RX) {
            ...
            i2s_hal_set_in_link(&(p_i2s_obj[i2s_num]->hal),
                p_i2s_obj[i2s_num]->dma_buf_len * p_i2s_obj[i2s_num]->channel_num * p_i2s_obj[i2s_num]->bytes_per_sample,
    ...
    if(p_i2s_obj[i2s_num]->use_apll && i2s_apll_calculate_fi2s(fi2s_clk, bits, &sdm0, &sdm1, &sdm2, &odir) == ESP_OK) {
        ...
        ESP_LOGI(I2S_TAG, "APLL: Req RATE: %d, real rate: %0.3f, BITS: %u, CLKM: %u, BCK_M: %u, MCLK: %0.3f, SCLK: %f, diva: %d, divb: %d", rate,
            fi2s_rate / bits / channel / m_scale, bits, 1, m_scale, fi2s_rate, fi2s_rate / 8, 1, 0);
    } else {
        ...
        ESP_LOGI(I2S_TAG, "PLL_D2: Req RATE: %d, real rate: %0.3f, BITS: %u, CLKM: %u, BCK: %u, MCLK: %0.3f, SCLK: %f, diva: %d, divb: %d", rate,
            real_rate, bits, clkmInteger, bck, (double)I2S_BASE_CLK / mclk, real_rate * bits * channel, 64, clkmDecimals);
    }
    if(p_i2s_obj[i2s_num]->mode & I2S_MODE_TX) {
    ...
    if((p_i2s_obj[i2s_num]->mode & I2S_MODE_TX) && p_i2s_obj[i2s_num]->tx) { xSemaphoreGive(p_i2s_obj[i2s_num]->tx->mux); }
    if((p_i2s_obj[i2s_num]->mode & I2S_MODE_RX) && p_i2s_obj[i2s_num]->rx) { xSemaphoreGive(p_i2s_obj[i2s_num]->rx->mux); }
    i2s_start(i2s_num);
    return ESP_OK;
}
...
static void IRAM_ATTR i2s_intr_handler_default(void* arg) {
    ...
    if((status & I2S_INTR_OUT_DSCR_ERR) || (status & I2S_INTR_IN_DSCR_ERR)) {
        ...
        if(p_i2s->i2s_queue) {
            ...
            if(xQueueIsQueueFullFromISR(p_i2s->i2s_queue)) { xQueueReceiveFromISR(p_i2s->i2s_queue, &dummy, &high_priority_task_awoken); }
    ...
    if((status & I2S_INTR_OUT_EOF) && p_i2s->tx) {
        ...
        if(xQueueIsQueueFullFromISR(p_i2s->tx->queue)) {
            ...
            if(p_i2s->tx_desc_auto_clear == true) { memset((void*)dummy, 0, p_i2s->tx->buf_size); }
        }
    ...
    if((status & I2S_INTR_IN_SUC_EOF) && p_i2s->rx) {
        ...
        if(xQueueIsQueueFullFromISR(p_i2s->rx->queue)) { xQueueReceiveFromISR(p_i2s->rx->queue, &dummy, &high_priority_task_awoken); }
...
static esp_err_t i2s_destroy_dma_queue(i2s_port_t i2s_num, i2s_dma_t* dma) {
    ...
    for(bux_idx = 0; bux_idx < p_i2s_obj[i2s_num]->dma_buf_count; bux_idx++) {
        if(dma->desc && dma->desc[bux_idx]) { free(dma->desc[bux_idx]); }
        if(dma->buf && dma->buf[bux_idx]) { free(dma->buf[bux_idx]); }
    }
    if(dma->buf) { free(dma->buf); }
    if(dma->desc) { free(dma->desc); }
...
static esp_err_t i2s_check_cfg_static(i2s_port_t i2s_num, const i2s_config_t* cfg) {
    ...
    I2S_CHECK(
        cfg->communication_format && (cfg->communication_format < I2S_COMM_FORMAT_STAND_MAX), "invalid communication formats", ESP_ERR_INVALID_ARG);
    I2S_CHECK(!((cfg->communication_format & I2S_COMM_FORMAT_STAND_MSB) && (cfg->communication_format & I2S_COMM_FORMAT_STAND_PCM_LONG)),
...
esp_err_t i2s_zero_dma_buffer(i2s_port_t i2s_num) {
    ...
    if(p_i2s_obj[i2s_num]->rx && p_i2s_obj[i2s_num]->rx->buf != NULL && p_i2s_obj[i2s_num]->rx->buf_size != 0) {
        for(int i = 0; i < p_i2s_obj[i2s_num]->dma_buf_count; i++) { memset(p_i2s_obj[i2s_num]->rx->buf[i], 0, p_i2s_obj[i2s_num]->rx->buf_size); }
    }
    if(p_i2s_obj[i2s_num]->tx && p_i2s_obj[i2s_num]->tx->buf != NULL && p_i2s_obj[i2s_num]->tx->buf_size != 0) {
        ...
        for(int i = 0; i < p_i2s_obj[i2s_num]->dma_buf_count; i++) { memset(p_i2s_obj[i2s_num]->tx->buf[i], 0, p_i2s_obj[i2s_num]->tx->buf_size); }
    }
    return ESP_OK;
}
...
esp_err_t i2s_driver_install(i2s_port_t i2s_num, const i2s_config_t* i2s_config, int queue_size, void* i2s_queue) {
    ...
    I2S_CHECK(
        (i2s_config->dma_buf_count >= 2 && i2s_config->dma_buf_count <= 128), "I2S buffer count less than 128 and more than 2", ESP_ERR_INVALID_ARG);
    I2S_CHECK(
        (i2s_config->dma_buf_len >= 8 && i2s_config->dma_buf_len <= 1024), "I2S buffer length at most 1024 and more than 8", ESP_ERR_INVALID_ARG);
    if(p_i2s_obj[i2s_num] == NULL) {
        ...
        for(int x = 0; x < I2S_NUM_MAX; x++) { i2s_spinlock[x] = i2s_spinlock_unlocked[0]; }
        ...
        //initial interrupt
        err = i2s_isr_register(
            i2s_num, i2s_config->intr_alloc_flags, i2s_intr_handler_default, p_i2s_obj[i2s_num], &p_i2s_obj[i2s_num]->i2s_isr_handle);
        if(err != ESP_OK) {
#ifdef CONFIG_PM_ENABLE
            if(p_i2s_obj[i2s_num]->pm_lock) { esp_pm_lock_delete(p_i2s_obj[i2s_num]->pm_lock); }
#endif
            free(p_i2s_obj[i2s_num]);
            p_i2s_obj[i2s_num] = NULL;
    ...
    *bytes_written = 0;
    ...
    while(size > 0) {
        if(p_i2s_obj[i2s_num]->tx->rw_pos == p_i2s_obj[i2s_num]->tx->buf_size || p_i2s_obj[i2s_num]->tx->curr_ptr == NULL) {
            if(xQueueReceive(p_i2s_obj[i2s_num]->tx->queue, &p_i2s_obj[i2s_num]->tx->curr_ptr, ticks_to_wait) == pdFALSE) { break; }
            p_i2s_obj[i2s_num]->tx->rw_pos = 0;
        }
        ESP_LOGD(I2S_TAG, "size: %d, rw_pos: %d, buf_size: %d, curr_ptr: %d", size, p_i2s_obj[i2s_num]->tx->rw_pos, p_i2s_obj[i2s_num]->tx->buf_size,
...
esp_err_t i2s_write_expand(
    i2s_port_t i2s_num, const void* src, size_t size, size_t src_bits, size_t aim_bits, size_t* bytes_written, TickType_t ticks_to_wait) {
    char* data_ptr;
    ...
    *bytes_written = 0;
    ...
    while(size > 0) {
        ...
        data_ptr += p_i2s_obj[i2s_num]->tx->rw_pos;
        bytes_can_write = p_i2s_obj[i2s_num]->tx->buf_size - p_i2s_obj[i2s_num]->tx->rw_pos;
        if(bytes_can_write > (int)size) { bytes_can_write = size; }
    ...
    *bytes_read = 0;
    ...
    while(size > 0) {
        if(p_i2s_obj[i2s_num]->rx->rw_pos == p_i2s_obj[i2s_num]->rx->buf_size || p_i2s_obj[i2s_num]->rx->curr_ptr == NULL) {
            if(xQueueReceive(p_i2s_obj[i2s_num]->rx->queue, &p_i2s_obj[i2s_num]->rx->curr_ptr, ticks_to_wait) == pdFALSE) { break; }
            p_i2s_obj[i2s_num]->rx->rw_pos = 0;
        }
        ...
        if(bytes_can_read > (int)size) { bytes_can_read = size; }
