...
bool is_dac_config_locked() {
    ...
    if(strlen(CONFIG_DAC_CONFIG) > 0) { return true; }
#endif
...
const sys_i2c_bus* get_i2c_bus(i2c_port_t port) {
    if(platform->dev.has_i2c && port == platform->dev.i2c.port - sys_i2c_port_PORT0 && platform->dev.i2c.scl >= 0) { return &platform->dev.i2c; }
    if(platform->dev.has_dac && platform->dev.dac.has_i2c && (platform->dev.dac.i2c.port - sys_i2c_port_PORT0) == port &&
        platform->dev.dac.i2c.scl >= 0) {
...
const i2c_config_t* config_i2c_get(sys_i2c_bus* bus) {
    ...
    if(bus && bus->scl >= 0 && bus->sda >= 0) {
        ...
        if(bus->speed > 0) { i2c.master.clk_speed = bus->speed; }
    }
...
const spi_bus_config_t* config_spi_get(spi_host_device_t* spi_host) {
    ...
    static spi_bus_config_t spi = {.mosi_io_num = -1, .sclk_io_num = -1, .miso_io_num = -1, .quadwp_io_num = -1, .quadhd_io_num = -1};
    if(platform->has_dev && platform->dev.has_spi) {
        ...
        if(platform->dev.spi.host != sys_dev_common_hosts_NONE) { spi_system_host = platform->dev.spi.host - sys_dev_common_hosts_Host0; }
    } else {
