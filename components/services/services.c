...
bool are_GPIOExp_equal(const sys_exp_config* exp1, const sys_exp_config* exp2) {
    ...
    if(exp1->model != exp2->model || exp1->addr != exp2->addr || exp1->base != exp2->base) { return false; }

    // Check if intr structure (pin and level) are the same
    if(exp1->intr != exp2->intr) { return false; }
...
bool sys_dev_config_callback(pb_istream_t* istream, pb_ostream_t* ostream, const pb_field_iter_t* field) {
    ...
    if(istream != NULL && field->tag == sys_dev_config_gpio_exp_tag) {
        ...
        if(!pb_decode(istream, &sys_exp_config_msg, &entry)) { return false; }
        ...
        *pExp = heap_caps_realloc(*pExp, sizeof(sys_exp_config) * gpio_exp_count, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        ...
        for(int i = 0; i < gpio_exp_count; i++) {
            ...
            if(!pb_encode_submessage(ostream, &sys_exp_config_msg, &(*pExp)[i])) { return false; }
        }
...
static void sleep_timer(uint32_t now) {
    ...
    if(sleep_context.spurious && now > sleep_context.spurious + first) {
        ...
        for(uint32_t (**sleeper)(void) = sleep_context.sleeper; *sleeper && spurious; sleeper++) { spurious &= (*sleeper)() >= now - first; }
...
void services_sleep_init(void) {
    ...
    // when moving to esp-idf more recent than 4.4.x, multiple gpio wake-up with level specific can
    // be done
    if(sleep_context.rtc_gpio) { ESP_LOGI(TAG, "RTC forced gpio bitmap 0x%llx (active 0x%llx)", sleep_context.rtc_gpio, sleep_context.rtc_level); }
    ...
    if(cause == ESP_SLEEP_WAKEUP_EXT0 || cause == ESP_SLEEP_WAKEUP_EXT1) {
        ...
        if(infrared_gpio() >= 0 && ((1LL << infrared_gpio()) & wake_gpio)) {
            ...
            if(sleep_config->spurious > 0) { sleep_context.spurious = sleep_config->spurious; }
...
void services_ports_init(void) {
    ...
    if(spi_config->mosi_io_num != -1 && spi_config->sclk_io_num != -1) {
        ...
        spi_system_host = -1;
        if(platform->dev.has_display && platform->dev.display.has_common && platform->dev.display.common.driver != sys_display_drivers_UNSPECIFIED &&
    ...
    if(platform->has_dev && gpio_exp_count > 0 && platform->dev.gpio_exp[0].model != sys_exp_models_UNSPECIFIED_EXP) {
        ...
        for(int count = 0; count < gpio_exp_count; count++) {
            ...
            if(exp->which_ExpType == sys_exp_config_spi_tag) {
                ...
                gpio_exp_config.phy.host = (!platform->dev.has_spi || (platform->dev.has_spi && platform->dev.spi.host == sys_dev_common_hosts_NONE)
                                                   ? sys_dev_common_hosts_Host0
                                                   : platform->dev.spi.host) -
                                           sys_dev_common_hosts_Host0;
                ...
                gpio_exp_config.phy.port = ((!platform->dev.has_i2c || (platform->dev.has_i2c && platform->dev.i2c.port == sys_i2c_port_UNSPECIFIED))
                                                   ? sys_dev_common_ports_SYSTEM
                                                   : platform->dev.i2c.port) -
                                           sys_dev_common_ports_SYSTEM;
            }
