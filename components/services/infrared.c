...
static esp_err_t nec_parser_get_scan_code(ir_parser_t* parser, uint32_t* address, uint32_t* command, bool* repeat) {
    ...
    if(nec_parser->repeat) {
        ...
        if(nec_parse_head(nec_parser)) {
            ...
            for(int i = 15; i >= 0; i--) {
                if(nec_parse_logic(parser, &logic_value) == ESP_OK) { addr |= (logic_value << i); }
            }
            for(int i = 15; i >= 0; i--) {
                if(nec_parse_logic(parser, &logic_value) == ESP_OK) { cmd |= (logic_value << i); }
            }
            *address = addr;
            *command = cmd;
            *repeat = false;
...
ir_parser_t* ir_parser_rmt_new_nec(const ir_parser_config_t* config) {
    ...
    RMT_CHECK(rmt_get_counter_clock((rmt_channel_t)config->dev_hdl, &counter_clk_hz) == ESP_OK, "get rmt counter clock failed", err, NULL);
...
static esp_err_t rc5_parser_get_scan_code(ir_parser_t* parser, uint32_t* address, uint32_t* command, bool* repeat) {
    ...
    for(int i = 0; i < rc5_parser->buffer_len; i++) {
        if(rc5_duration_one_unit(rc5_parser, rc5_parser->buffer[i].duration0)) {
            ...
            if(rc5_duration_two_unit(rc5_parser, rc5_parser->buffer[i].duration1)) { exchange = !exchange; }
        } else if(rc5_duration_two_unit(rc5_parser, rc5_parser->buffer[i].duration0)) {
            parse_result <<= 1;
            parse_result |= rc5_parser->buffer[i].level0;
            parse_result <<= 1;
            parse_result |= !rc5_parser->buffer[i].level0;
            if(rc5_duration_one_unit(rc5_parser, rc5_parser->buffer[i].duration1)) { exchange = !exchange; }
        } else {
            goto out;
        }
    }
    if(!(rc5_parser->flags & IR_TOOLS_FLAGS_INVERSE)) { parse_result = ~parse_result; }
 ...
 * 
 */
int8_t infrared_gpio(void) { return ir_gpio; };
