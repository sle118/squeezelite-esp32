 ...
 */
uint32_t gpio_exp_get_base(gpio_exp_t* expander) { return expander->first; }
...
gpio_exp_t* gpio_exp_create(const gpio_exp_config_t* config) {
    ...
    ESP_LOGI(TAG, "Create GPIO expander %s at base %u with intr %d at @%x on port/host %d/%d, enable pin: %d:%d", config->model, config->base,
        config->intr, config->phy.addr, config->phy.port, config->phy.host, config->phy.ena_pin, config->phy.ena_lvl);
    return expander;
}
...
esp_err_t gpio_exp_isr_handler_add(int gpio, gpio_isr_t isr_handler, uint32_t debounce, void* arg, struct gpio_exp_s* expander) {
    ...
    if(debounce)
        expander->isr[gpio].timer = xTimerCreate("gpioExpDebounce", pdMS_TO_TICKS(debounce), pdFALSE, expander->isr + gpio, debounce_handler);
...
static void IRAM_ATTR intr_isr_handler(void* arg) {
    ...
    for(int i = 0; i < n_expanders; i++)
        if(expanders[i].intr == self->intr) expanders[i].intr_pending = true;
        ...
        if(notif == GPIO_EXP_INTR) {
            ...
            for(int i = 0; i < n_expanders; i++) {
                ...
                for(int gpio = 31, clz = 0; pending && clz < 31; pending <<= (clz + 1)) {
                    ...
                    gpio -= clz;
                    if(expander->isr[gpio].timer)
                        xTimerReset(expander->isr[gpio].timer, 1); // todo 0
                    else if(expander->isr[gpio].handler)
                        expander->isr[gpio].handler(expander->isr[gpio].arg);
                }
            }
        }
...
static esp_err_t mpr121_init(gpio_exp_t* self) {
    static const struct {
        uint8_t addr;
        uint8_t data;
    } mpr121_init_table[] = {{0x80, 0x63}, /* Soft reset */
...
static void mpr121_write(gpio_exp_t* self) { ESP_LOGE(TAG, "MPR121 GPIO write not implemented"); }

/****************************************************************************************
 * PCA9535 family : direction, read and write
 */
static void pca9535_set_direction(gpio_exp_t* self) { i2c_write(self->phy.port, self->phy.addr, 0x06, self->r_mask, 2); }

static uint32_t pca9535_read(gpio_exp_t* self) { return i2c_read(self->phy.port, self->phy.addr, 0x00, 2); }

static void pca9535_write(gpio_exp_t* self) { i2c_write(self->phy.port, self->phy.addr, 0x02, self->shadow, 2); }
...
static void mcp23017_set_pull_mode(gpio_exp_t* self) { i2c_write(self->phy.port, self->phy.addr, 0x0c, self->pullup, 2); }
...
static void mcp23017_write(gpio_exp_t* self) { i2c_write(self->phy.port, self->phy.addr, 0x12, self->shadow, 2); }
...
static void mcp23s17_set_pull_mode(gpio_exp_t* self) { spi_write(self->spi_handle, self->phy.addr, 0x0c, self->pullup, 2); }
...
static void mcp23s17_write(gpio_exp_t* self) { spi_write(self->spi_handle, self->phy.addr, 0x12, self->shadow, 2); }
...
static esp_err_t i2c_write(uint8_t port, uint8_t addr, uint8_t reg, uint32_t data, int len) {
    ...
    if(len > 1)
        i2c_master_write(cmd, (uint8_t*)&data, len, I2C_MASTER_NACK);
    else
    ...
    if(ret != ESP_OK) { ESP_LOGW(TAG, "I2C write failed"); }
...
static uint32_t i2c_read(uint8_t port, uint8_t addr, uint8_t reg, int len) {
    ...
    if(ret != ESP_OK) { ESP_LOGW(TAG, "I2C read failed"); }
