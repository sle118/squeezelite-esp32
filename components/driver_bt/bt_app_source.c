...
void set_a2dp_media_state(int new_state) {
    if(bt_app_source_media_state != new_state) { bt_app_source_media_state = new_state; }
}
...
static void handle_bt_gap_pin_req(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t* param) {
    ...
    if(pin_str && ((!param->pin_req.min_16_digit && pinlen == 4) || (param->pin_req.min_16_digit && pinlen == 16))) {
        ...
        for(size_t i = 0; i < pinlen; i++) { pin_code[i] = pin_str[i]; }
...
int heart_beat_delay[] = {1000, 1000, 1000, 1000, 10000, 500, 1000};
...
static void unexpected_connection_state(int from, esp_a2d_connection_state_t to) {
    ESP_LOGW(TAG, "Unexpected connection state change. App State: %s (%u) Connection State %s (%u)", sys_status_av_states_name(from), from,
        conn_state_str(to), to);
}
...
static void handle_connect_state_unconnected(uint16_t event, esp_a2d_cb_param_t* param) {
    ...
    case ESP_A2D_CONNECTION_STATE_CONNECTED:
        unexpected_connection_state(bt_app_source_a2d_state, param->conn_stat.state);
        ESP_LOGE(TAG, "Connection state event received while status was unconnected.  Routing message to connecting state handler. State : %u",
            param->conn_stat.state);
        if(param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) { handle_connect_state_connecting(event, param); }
        break;
    case ESP_A2D_CONNECTION_STATE_DISCONNECTING:
        unexpected_connection_state(bt_app_source_a2d_state, param->conn_stat.state);
        break;
...
static void bt_app_av_sm_hdlr(uint16_t event, void* param) {
    ESP_LOGV(TAG, "bt_app_av_sm_hdlr.%s a2d state: %s", event == BT_APP_HEART_BEAT_EVT ? "Heart Beat." : "",
        sys_status_av_states_name(bt_app_source_a2d_state));
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t* param) {
    ...
    if(strlen((char*)s_peer_bdname) > 0) { peers_list_maintain((const char*)s_peer_bdname, rssi); }
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void* p_param) {
    ...
    case ESP_AVRC_CT_CONNECTION_STATE_EVT: {
        ...
        ESP_LOGI(BT_RC_CT_TAG, "AVRC conn_state evt: state %d, [%02x:%02x:%02x:%02x:%02x:%02x]", rc->conn_stat.connected, bda[0], bda[1], bda[2],
            bda[3], bda[4], bda[5]);
