...
typedef enum { OTA_TYPE_HTTP, OTA_TYPE_BUFFER, OTA_TYPE_INVALID } ota_type_t;
...
uint8_t ota_get_pct_complete() {
    return ota_status->total_image_len == 0 ? 0 : (uint8_t)((float)ota_status->actual_image_len / ota_status->total_image_len * 100.0f);
}
uint8_t ota_get_pct_downloaded() {
    return ota_status->total_image_len == 0 ? 0 : (uint8_t)(ota_status->downloaded_image_len / ota_status->total_image_len * 100.0f);
}
typedef struct {
...
static void loc_displayer_progressbar(uint8_t pct) {
    ...
    if(display) {
        ...
        ESP_LOGD(TAG, "Drawing %d,%d,%d,%d", progress_coordinates->border.x1, progress_coordinates->border.y1, progress_coordinates->border.x2,
            progress_coordinates->border.y2);
        GDS_DrawBox(display, progress_coordinates->border.x1, progress_coordinates->border.y1, progress_coordinates->border.x2,
            progress_coordinates->border.y2, GDS_COLOR_WHITE, false);
        ESP_LOGD(
            TAG, "Drawing %d,%d,%d,%d", progress_coordinates->filler.x1, progress_coordinates->filler.y1, filler_x, progress_coordinates->filler.y2);
        if(filler_x > progress_coordinates->filler.x1) {
            GDS_DrawBox(display, progress_coordinates->filler.x1, progress_coordinates->filler.y1, filler_x, progress_coordinates->filler.y2,
                GDS_COLOR_WHITE, true);
        } else {
            // Clear the inner box
            GDS_DrawBox(display, progress_coordinates->filler.x1, progress_coordinates->filler.y1, progress_coordinates->filler.x2,
                progress_coordinates->filler.y2, GDS_COLOR_BLACK, true);
        }
        ESP_LOGD(TAG, "Updating Display");
        GDS_Update(display);
    }
    if(led_display) { led_vu_progress_bar(pct, LED_VU_BRIGHT); }
}
void sendMessaging(messaging_types type, const char* fmt, ...) {
    ...
    va_end(args);
    if(type != MESSAGING_INFO) { IF_DISPLAY(GDS_TextLine(display, 2, GDS_TEXT_LEFT, GDS_TEXT_CLEAR | GDS_TEXT_UPDATE, msg_str)); }
...
esp_err_t handle_http_on_data(esp_http_client_event_t* evt) {
    ...
    if(http_status == 200) {
        ...
        if(ota_get_pct_downloaded() % 5 == 0 && ota_get_pct_downloaded() % 5 != ota_status->newdownloadpct) {
            ...
            ESP_LOGI(TAG, "OTA download progress : %f/%f (%d pct), %f KB/s", ota_status->downloaded_image_len, ota_status->total_image_len,
                ota_status->newdownloadpct, elapsed_ms > 0 ? ota_status->downloaded_image_len * 1000 / elapsed_ms / 1024 : 0);
...
esp_err_t _erase_last_boot_app_partition(const esp_partition_t* ota_partition) {
    ...
    if(single_pass_size % SPI_FLASH_SEC_SIZE != 0) {
        ...
        ESP_LOGW(TAG, "Invalid erase block size of %u. Value should be a multiple of %d and will be adjusted to %u.", single_pass_size,
            SPI_FLASH_SEC_SIZE, temp_single_pass_size);
    ...
    for(uint16_t i = 0; i < num_passes; i++) {
        ...
        ESP_LOGD(TAG, "Pass %d of %d, with chunks of %d bytes, from %d to %d", i + 1, num_passes, single_pass_size, i * single_pass_size,
            i * single_pass_size + single_pass_size);
...
esp_err_t ota_header_check() {
    ...
    ESP_LOGD(TAG, "Running partition [%s] type %d subtype %d (offset 0x%08x)", ota_status->running->label, ota_status->running->type,
        ota_status->running->subtype, ota_status->running->address);
    ...
    if(ota_status->configured != ota_status->running) {
        ESP_LOGW(TAG, "Configured OTA boot partition at offset 0x%08x, but running from offset 0x%08x", ota_status->configured->address,
            ota_status->running->address);
        ESP_LOGW(TAG, "(This can happen if either the OTA boot data or preferred boot image become corrupted somehow.)");
    }
    ESP_LOGD(TAG, "Next ota update partition is: [%s] subtype %d at offset 0x%x", ota_status->update_partition->label,
        ota_status->update_partition->subtype, ota_status->update_partition->address);
    ...
    while(ota_status->remain_image_len > 0) {
        ...
        if(data_read <= 0) {
            ...
            if(ota_status->lastpct != ota_status->newpct) {
                ...
                gettimeofday(&tv, NULL);
                uint32_t elapsed_ms = (tv.tv_sec - ota_status->OTA_start.tv_sec) * 1000 + (tv.tv_usec - ota_status->OTA_start.tv_usec) / 1000;
                ESP_LOGI(TAG, "OTA progress : %d/%.0f (%d pct), %d KB/s", ota_status->actual_image_len, ota_status->total_image_len,
                    ota_status->newpct, elapsed_ms > 0 ? ota_status->actual_image_len * 1000 / elapsed_ms / 1024 : 0);
...
esp_err_t process_recovery_ota(const char* bin_url, char* bin_buffer, uint32_t length) {
    ...
    ESP_LOGD(TAG, "OTA task stack size %d, priority %d (%d %s ESP_TASK_MAIN_PRIO)", stack_size, task_priority,
        abs(task_priority - ESP_TASK_MAIN_PRIO), task_priority - ESP_TASK_MAIN_PRIO > 0 ? "above" : "below");
